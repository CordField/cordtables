...
class CreateStageRoleColumnGrantExResponse extends GenericResponse {
  stageRoleColumnGrant: CommonStageRoleColumnGrant;
}

class CommonStageRoleColumnGrantListRequest {
  token: string;
}

class CommonStageRoleColumnGrantListResponse {
  error: ErrorType;
  stageRoleColumnGrants: CommonStageRoleColumnGrant[];
}


class CommonStageRoleColumnGrantUpdateRequest {
  token: string;
  column: string;
  value: any;
  id: string;
...
class CommonStageRoleColumnGrantUpdateResponse {
  error: ErrorType;
  stageRoleColumnGrant: CommonStageRoleColumnGrant | null = null;
}
...
@Component({
  tag: 'common-stage-role-column-grants',
  styleUrl: 'common-stage-role-column-grants.css',
  shadow: true,
})
export class CommonStageRoleColumnGrants {
  ...
  handleUpdate = async (id: string, columnName: string, value: string): Promise<boolean> => {
    ...
    if (updateResponse.error == ErrorType.NoError) {
      this.stageRoleColumnGrantsResponse = { error: ErrorType.NoError, stageRoleColumnGrants: this.stageRoleColumnGrantsResponse.stageRoleColumnGrants.map(stageRoleColumnGrant => (stageRoleColumnGrant.id === id ? updateResponse.stageRoleColumnGrant : stageRoleColumnGrant)) };
  ...
  handleDelete = async id => {
    const deleteResponse = await fetchAs<DeleteStageRoleColumnGrantExRequest, DeleteStageRoleColumnGrantExResponse>('common-stage-role-column-grants/delete', {
      id,
      token: globals.globalStore.state.token,
    });
  ...
  handleInsert = async (event: MouseEvent) => {
    ...
    const createResponse = await fetchAs<CreateStageRoleColumnGrantExRequest, CreateStageRoleColumnGrantExResponse>('common-stage-role-column-grants/create-read', {
      token: globals.globalStore.state.token,
      stageRoleColumnGrant: {
        stage: this.newStage,
        role: this.newRole,
  ...
  render() {
    return (
      <Host>
        <slot></slot>
        {/* table abstraction */}
        {this.stageRoleColumnGrantsResponse && <cf-table rowData={this.stageRoleColumnGrantsResponse.stageRoleColumnGrants} columnData={this.columnData}></cf-table>}
