...
export class CommonStageNotifications {
  ...
  async getList() {
    ...
    if (this.stageNotificationsResponse.error === ErrorType.NoError) {
      await this.updateForeignKeys();
    }
  }

  async updateForeignKeys() {
    for (const stageNotification of this.stageNotificationsResponse.stageNotifications) {
      for (const column of this.columnData) {
        if (column.foreignKey !== null && column.foreignKey !== undefined) {
          const autocompleteData = await fetchAs<AutocompleteRequest, AutocompleteResponse>('admin/autocomplete', {
            ...
            searchKeyword: stageNotification[column.field],
          });
          console.log(autocompleteData);
          if (autocompleteData.error === ErrorType.NoError) {
            this.stageNotificationsResponse.stageNotifications.map(stageNotification2 => {
              if (stageNotification.id === stageNotification2.id) {
                stageNotification2[column.field] = {
                  value: stageNotification[column.field],
                  displayValue: autocompleteData.data,
                };
              }
              return stageNotification2;
            });
          }
        }
      }
