...
class CreateNoteExResponse extends GenericResponse {
  note: CommonNote;
}

class CommonNoteListRequest {
  token: string;
}

class CommonNoteListResponse {
  error: ErrorType;
  notes: CommonNote[];
}
...
export class CommonNotes {
  ...
  handleUpdate = async (id: number, columnName: string, value: string): Promise<boolean> => {
    ...
    if (updateResponse.error == ErrorType.NoError) {
      this.notesResponse = { error: ErrorType.NoError, notes: this.notesResponse.notes.map(note => (note.id === id ? updateResponse.note : note)) };
  ...
  render() {
    return (
      <Host>
        <slot></slot>
        {/* table abstraction */}
        {this.notesResponse && <cf-table rowData={this.notesResponse.notes} columnData={this.columnData}></cf-table>}
