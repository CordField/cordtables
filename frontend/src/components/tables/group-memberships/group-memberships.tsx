...
export class GroupMemberships {
  ...
  async getList() {
    this.listResponse = await fetchAs<GroupMembershipsListRequest, GroupMembershipsListResponse>('group_memberships/list', { token: globals.globalStore.state.token });
  }
  ...
  submit = async () => {
    this.createResponse = await fetchAs<GroupCreateRequest, GroupCreateResponse>('group_memberships/create', {
  ...
  updateName = async (id: number, value: string): Promise<boolean> => {
    this.createResponse = await fetchAs<GroupUpdateRequest, GroupUpdateResponse>('group_memberships/update', { token: globals.globalStore.state.token, name: value, id });
  ...
  clickRemoveRowIcon = async (value: number): Promise<boolean> => {
    this.deleteResponse = await fetchAs<GroupDeleteRequest, GroupDeleteResponse>('group_memberships/delete', { token: globals.globalStore.state.token, id: value });
