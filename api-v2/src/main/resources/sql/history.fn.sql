...
create or replace procedure create_history_tables()
language plpgsql
as $$
declare
	rec1 record;
...
begin
	...
	order by table_name) loop
		...
		_history_created_at timestamp not null default CURRENT_TIMESTAMP, __event history_event_type)', rec1.table_schema,history_table_name);
...
begin
	...
	and table_schema <> 'information_schema'
	and table_type <> 'VIEW'
	and table_name not similar to '%(_peer|_history|_x)'
	order by table_name) loop
...
call create_history_tables();
call create_history_triggers();
