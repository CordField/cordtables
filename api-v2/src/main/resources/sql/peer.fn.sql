...
create or replace procedure create_peer_tables()
language plpgsql
as $$
declare
	rec0 record;
...
begin
    ...
    and table_schema <> 'information_schema' ) loop
        ...
        where table_schema = rec0.table_schema and table_name not similar to '%(_peer|_history|_x)'
        order by table_name) loop
            ...
            execute format('create table if not exists %I.%I ( _peer_id varchar(32) not null references admin_peers(id),
