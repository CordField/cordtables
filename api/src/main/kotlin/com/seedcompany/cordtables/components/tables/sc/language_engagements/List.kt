...
data class ScLanguageEngagementsListRequest(
    val token: String?
)

data class ScLanguageEngagementsListResponse(
    val error: ErrorType,
    val languageEngagements: MutableList<languageEngagement>?
)

@CrossOrigin(origins = ["http://localhost:3333", "https://dev.cordtables.com", "https://cordtables.com", "*"])
@Controller("ScLanguageEngagementsList")
class List(
    @Autowired
    val util: Utility,
    ...
    @PostMapping("sc/language-engagements/list")
    @ResponseBody
    fun listHandler(@RequestBody req:ScLanguageEngagementsListRequest): ScLanguageEngagementsListResponse {
        var data: MutableList<languageEngagement> = mutableListOf()
        if (req.token == null) return ScLanguageEngagementsListResponse(ErrorType.TokenNotFound, mutableListOf())
        ...
        try {
            ...
            return ScLanguageEngagementsListResponse(ErrorType.SQLReadError, mutableListOf())
        }

        return ScLanguageEngagementsListResponse(ErrorType.NoError, data)
    }
}
