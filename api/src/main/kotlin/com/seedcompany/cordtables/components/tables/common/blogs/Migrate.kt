...
class Migrate(
    ...
    @PostMapping("migrate/common-blogs")
    @ResponseBody
    fun registerHandler() {
        ...
        if (token != null) {
            ...
            // get count of blog entries
            val countQuery = """{"query":"query Query {blogs {total}}"}"""
            val countResponse = cord.post<GResponse>(token, countQuery)
            val total = countResponse?.data?.blogs?.total

            if (total == null) {
                println("blog total was null")
                return
            }

            println("total blogs: $total")

            // iterate over all blogs and ensure we have them in postgres
            for (i in 1..total) {

                val langQuery = """
                        {"query":"query Query { blogs(input: { page: $i, count: 1 }) { items { id ethnologue { code { value } } avatarLetters name { value } displayName { value } displayNamePronunciation { value } isDialect { value } populationOverride { value } registryOfDialectsCode { value } leastOfThese { value } leastOfTheseReason { value } signBlogCode { value } sponsorEstimatedEndDate { value } sensitivity isSignBlog { value } hasExternalFirstScripture { value } tags { value } presetInventory { value } population { value } } } }"}
                    """.trimIndent()

                val langResponse = cord.post<GResponse>(token, langQuery)

                val lang = langResponse?.data?.blogs?.items?.get(0)

                if (lang == null) {
                    println("Blog entry was null")
                    continue
                }
                ...
                this.ds.connection.use { conn ->
                    val migrationStatement = conn.prepareCall("call sc.sc_migrate_blog(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);")
                    ...
                    migrationStatement.setString(11, lang.signBlogCode?.value)
                    ...
                    if(lang.isSignBlog?.value == null)
                        migrationStatement.setNull(14, java.sql.Types.NULL)
                    else
                        migrationStatement.setBoolean(14, lang.isSignBlog?.value)
                    ...
                    if (errorType != ErrorType.NoError) {
                        println("Blog migration query failed")
                    }

                    migrationStatement.close()
                }

                println("Migrated blog entry code: '${lang.id}': $errorType")
