...
data class CommonWorkflowsReadRequest(
    val token: String?,
    val id: Int? = null,
)

data class CommonWorkflowsReadResponse(
    val error: ErrorType,
    val workflow: workflow? = null,
)

@CrossOrigin(origins = ["http://localhost:3333", "https://dev.cordtables.com", "https://cordtables.com", "*"])
@Controller("CommonWorkflowsRead")
class Read(
    @Autowired
    val util: Utility,
    ...
    fun readHandler(@RequestBody req: CommonWorkflowsReadRequest): CommonWorkflowsReadResponse {
        ...
        if (req.token == null) return CommonWorkflowsReadResponse(ErrorType.TokenNotFound)
        if (req.id == null) return CommonWorkflowsReadResponse(ErrorType.MissingId)
        ...
        try {
            ...
            while (jdbcResult.next()) {
                ...
                return CommonWorkflowsReadResponse(ErrorType.NoError, workflow = workflow)

            }
        } catch (e: SQLException) {
            println("error while listing ${e.message}")
            return CommonWorkflowsReadResponse(ErrorType.SQLReadError)
        }

        return CommonWorkflowsReadResponse(error = ErrorType.UnknownError)
    }
}