...
class Delete(
    ...
    fun deleteHandler(@RequestBody req: CommonOrganizationsDeleteRequest): CommonOrganizationsDeleteResponse {
        ...
        var deletedOrganizationId: String? = null

        this.ds.connection.use { conn ->
            try {

                val deleteStatement = conn.prepareCall(
                        "delete from common.organizations where id = ?::uuid returning id"
                )
                ...
                if (deleteStatementResult.next()) {
                    deletedOrganizationId  = deleteStatementResult.getString("id")
                }
            }
