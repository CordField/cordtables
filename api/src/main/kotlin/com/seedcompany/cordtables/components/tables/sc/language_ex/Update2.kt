...
data class UpdateLanguageExRequest2(
    val token: String?,
    val languageEx: LanguageExInput? = null,
)

data class UpdateLanguageExResponse2(
    val error: ErrorType,
)
...
class Update2(
    ...
    @ResponseBody
    fun updateHandler(@RequestBody req: UpdateLanguageExRequest2): UpdateLanguageExResponse2 {

        if (req.token == null) return UpdateLanguageExResponse2(ErrorType.TokenNotFound)
        if (req.languageEx == null) return UpdateLanguageExResponse2(ErrorType.MissingId)
        if (req.languageEx.id == null) return UpdateLanguageExResponse2(ErrorType.MissingId)

        if (req.languageEx.egids_level != null && !enumContains<EgidsScale>(req.languageEx.egids_level)) {
            return UpdateLanguageExResponse2(
        ...
        return UpdateLanguageExResponse2(ErrorType.NoError)
    }
}
