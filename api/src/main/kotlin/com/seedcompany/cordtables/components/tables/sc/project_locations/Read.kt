...
data class ScProjectLocationsReadRequest(
    val token: String?,
    val id: Int? = null,
)

data class ScProjectLocationsReadResponse(
    val error: ErrorType,
    val projectLocation: projectLocation? = null,
)

@CrossOrigin(origins = ["http://localhost:3333", "https://dev.cordtables.com", "https://cordtables.com", "*"])
@Controller("ScProjectLocationsRead")
class Read(
    @Autowired
    val util: Utility,
    ...
    @PostMapping("sc/project-locations/read")
    @ResponseBody
    fun readHandler(@RequestBody req: ScProjectLocationsReadRequest): ScProjectLocationsReadResponse {

        if (req.token == null) return ScProjectLocationsReadResponse(ErrorType.TokenNotFound)
        if (req.id == null) return ScProjectLocationsReadResponse(ErrorType.MissingId)
        ...
        try {
            ...
            while (jdbcResult.next()) {
                ...
                return ScProjectLocationsReadResponse(ErrorType.NoError, projectLocation = projectLocation)

            }
        } catch (e: SQLException) {
            println("error while listing ${e.message}")
            return ScProjectLocationsReadResponse(ErrorType.SQLReadError)
        }

        return ScProjectLocationsReadResponse(error = ErrorType.UnknownError)
    }
}
