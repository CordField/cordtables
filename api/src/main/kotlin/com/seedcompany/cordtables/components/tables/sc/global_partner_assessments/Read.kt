...
@Controller("ScGlobalPartnerAssessmentsRead")
class Read(
    @Autowired
    val util: Utility,
    ...
    @PostMapping("sc-global-partner-assessments/read")
    @ResponseBody
    fun readHandler(@RequestBody req: ScGlobalPartnerAssessmentsReadRequest): ScGlobalPartnerAssessmentsReadResponse {

        if (req.token == null) return ScGlobalPartnerAssessmentsReadResponse(ErrorType.TokenNotFound)
        if (req.id == null) return ScGlobalPartnerAssessmentsReadResponse(ErrorType.MissingId)
            ...
            while (jdbcResult.next()) {
                ...
                val globalPartnerAssessment =
                    globalPartnerAssessment(
                        id = id,
                        partner = partner,
                        governance_trans = if (governance_trans == null) null else PartnerMaturityScale.valueOf(governance_trans),
                        director_trans = if (director_trans == null) null else PartnerMaturityScale.valueOf(director_trans),
                        identity_trans = if (identity_trans == null) null else PartnerMaturityScale.valueOf(identity_trans),
                        growth_trans = if (growth_trans == null) null else PartnerMaturityScale.valueOf(growth_trans),
                        comm_support_trans = if (comm_support_trans == null) null else PartnerMaturityScale.valueOf(comm_support_trans),
                        systems_trans = if (systems_trans == null) null else PartnerMaturityScale.valueOf(systems_trans),
                        fin_management_trans = if (fin_management_trans == null) null else PartnerMaturityScale.valueOf(fin_management_trans),
                        hr_trans = if (hr_trans == null) null else PartnerMaturityScale.valueOf(hr_trans),
                        it_trans = if (it_trans == null) null else PartnerMaturityScale.valueOf(it_trans),
                        program_design_trans = if (program_design_trans == null) null else PartnerMaturityScale.valueOf(program_design_trans),
                        tech_translation_trans = if (tech_translation_trans == null) null else PartnerMaturityScale.valueOf(tech_translation_trans),
                        director_opp = if (director_opp == null) null else PartnerMaturityScale.valueOf(director_opp),
                        financial_management_opp = if (financial_management_opp == null) null else PartnerMaturityScale.valueOf(financial_management_opp),
                        program_design_opp = if (program_design_opp == null) null else PartnerMaturityScale.valueOf(program_design_opp),
                        tech_translation_opp = if (tech_translation_opp == null) null else PartnerMaturityScale.valueOf(tech_translation_opp),
                ...
                return ScGlobalPartnerAssessmentsReadResponse(ErrorType.NoError, globalPartnerAssessment = globalPartnerAssessment)

            }
        } catch (e: SQLException) {
            println("error while listing ${e.message}")
            return ScGlobalPartnerAssessmentsReadResponse(ErrorType.SQLReadError)
        }

        return ScGlobalPartnerAssessmentsReadResponse(error = ErrorType.UnknownError)
    }
}