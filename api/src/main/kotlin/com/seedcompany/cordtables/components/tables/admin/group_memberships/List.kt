...
data class AdminGroupMembershipsListResponse(
  val error: ErrorType,
  val groupMemberships: MutableList<groupMembership>? = null
)
...
class List(
  ...
  fun listHandler(@RequestBody req: AdminGroupMembershipsListRequest): AdminGroupMembershipsListResponse {
    ...
    if (req.token == null) return AdminGroupMembershipsListResponse(ErrorType.InputMissingToken)
    if (!util.isAdmin(req.token)) return AdminGroupMembershipsListResponse(ErrorType.AdminOnly)
