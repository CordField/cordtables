package com.seedcompany.cordtables.components.tables.sc.blogs
...
(
           ...
           where      a.table_name = 'common.blogs'
...
select
         ...
         case
                  when 'is_sign_blog' in
                           (
                                  select column_name
                                  from   column_level_access) then is_sign_blog
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then is_sign_blog
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then is_sign_blog
                  when 'is_sign_blog' in
                           (
                                  select column_name
                                  from   public_column_level_access) then is_sign_blog
                  else null
         end as is_sign_blog ,
         case
         ...
         case
                  when 'sign_blog_code' in
                           (
                                  select column_name
                                  from   column_level_access) then sign_blog_code
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then sign_blog_code
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then sign_blog_code
                  when 'sign_blog_code' in
                           (
                                  select column_name
                                  from   public_column_level_access) then sign_blog_code
                  else null
         end as sign_blog_code ,
         case
         ...
         case
                  when 'first_blog_population' in
                           (
                                  select column_name
                                  from   column_level_access) then first_blog_population
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then first_blog_population
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then first_blog_population
                  when 'first_blog_population' in
                           (
                                  select column_name
                                  from   public_column_level_access) then first_blog_population
                  else null
         end as first_blog_population ,
         case
         ...
         case
                  when 'multiple_blogs_leverage_linguistic_level' in
                           (
                                  select column_name
                                  from   column_level_access) then multiple_blogs_leverage_linguistic_level
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then multiple_blogs_leverage_linguistic_level
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then multiple_blogs_leverage_linguistic_level
                  when 'multiple_blogs_leverage_linguistic_level' in
                           (
                                  select column_name
                                  from   public_column_level_access) then multiple_blogs_leverage_linguistic_level
                  else null
         end as multiple_blogs_leverage_linguistic_level ,
         case
                  when 'multiple_blogs_leverage_linguistic_value' in
                           (
                                  select column_name
                                  from   column_level_access) then multiple_blogs_leverage_linguistic_value
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then multiple_blogs_leverage_linguistic_value
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then multiple_blogs_leverage_linguistic_value
                  when 'multiple_blogs_leverage_linguistic_value' in
                           (
                                  select column_name
                                  from   public_column_level_access) then multiple_blogs_leverage_linguistic_value
                  else null
         end as multiple_blogs_leverage_linguistic_value ,
         case
                  when 'multiple_blogs_leverage_linguistic_description' in
                           (
                                  select column_name
                                  from   column_level_access) then multiple_blogs_leverage_linguistic_description
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then multiple_blogs_leverage_linguistic_description
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then multiple_blogs_leverage_linguistic_description
                  when 'multiple_blogs_leverage_linguistic_description' in
                           (
                                  select column_name
                                  from   public_column_level_access) then multiple_blogs_leverage_linguistic_description
                  else null
         end as multiple_blogs_leverage_linguistic_description ,
         case
                  when 'multiple_blogs_leverage_linguistic_source' in
                           (
                                  select column_name
                                  from   column_level_access) then multiple_blogs_leverage_linguistic_source
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then multiple_blogs_leverage_linguistic_source
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then multiple_blogs_leverage_linguistic_source
                  when 'multiple_blogs_leverage_linguistic_source' in
                           (
                                  select column_name
                                  from   public_column_level_access) then multiple_blogs_leverage_linguistic_source
                  else null
         end as multiple_blogs_leverage_linguistic_source ,
         case
                  when 'multiple_blogs_leverage_joint_training_level' in
                           (
                                  select column_name
                                  from   column_level_access) then multiple_blogs_leverage_joint_training_level
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then multiple_blogs_leverage_joint_training_level
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then multiple_blogs_leverage_joint_training_level
                  when 'multiple_blogs_leverage_joint_training_level' in
                           (
                                  select column_name
                                  from   public_column_level_access) then multiple_blogs_leverage_joint_training_level
                  else null
         end as multiple_blogs_leverage_joint_training_level ,
         case
                  when 'multiple_blogs_leverage_joint_training_value' in
                           (
                                  select column_name
                                  from   column_level_access) then multiple_blogs_leverage_joint_training_value
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then multiple_blogs_leverage_joint_training_value
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then multiple_blogs_leverage_joint_training_value
                  when 'multiple_blogs_leverage_joint_training_value' in
                           (
                                  select column_name
                                  from   public_column_level_access) then multiple_blogs_leverage_joint_training_value
                  else null
         end as multiple_blogs_leverage_joint_training_value ,
         case
                  when 'multiple_blogs_leverage_joint_training_description' in
                           (
                                  select column_name
                                  from   column_level_access) then multiple_blogs_leverage_joint_training_description
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then multiple_blogs_leverage_joint_training_description
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then multiple_blogs_leverage_joint_training_description
                  when 'multiple_blogs_leverage_joint_training_description' in
                           (
                                  select column_name
                                  from   public_column_level_access) then multiple_blogs_leverage_joint_training_description
                  else null
         end as multiple_blogs_leverage_joint_training_description ,
         case
                  when 'multiple_blogs_leverage_joint_training_source' in
                           (
                                  select column_name
                                  from   column_level_access) then multiple_blogs_leverage_joint_training_source
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then multiple_blogs_leverage_joint_training_source
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then multiple_blogs_leverage_joint_training_source
                  when 'multiple_blogs_leverage_joint_training_source' in
                           (
                                  select column_name
                                  from   public_column_level_access) then multiple_blogs_leverage_joint_training_source
                  else null
         end as multiple_blogs_leverage_joint_training_source ,
         case
                  when 'lang_comm_int_in_blog_development_level' in
                           (
                                  select column_name
                                  from   column_level_access) then lang_comm_int_in_blog_development_level
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then lang_comm_int_in_blog_development_level
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then lang_comm_int_in_blog_development_level
                  when 'lang_comm_int_in_blog_development_level' in
                           (
                                  select column_name
                                  from   public_column_level_access) then lang_comm_int_in_blog_development_level
                  else null
         end as lang_comm_int_in_blog_development_level ,
         case
                  when 'lang_comm_int_in_blog_development_value' in
                           (
                                  select column_name
                                  from   column_level_access) then lang_comm_int_in_blog_development_value
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then lang_comm_int_in_blog_development_value
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then lang_comm_int_in_blog_development_value
                  when 'lang_comm_int_in_blog_development_value' in
                           (
                                  select column_name
                                  from   public_column_level_access) then lang_comm_int_in_blog_development_value
                  else null
         end as lang_comm_int_in_blog_development_value ,
         case
                  when 'lang_comm_int_in_blog_development_description' in
                           (
                                  select column_name
                                  from   column_level_access) then lang_comm_int_in_blog_development_description
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then lang_comm_int_in_blog_development_description
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then lang_comm_int_in_blog_development_description
                  when 'lang_comm_int_in_blog_development_description' in
                           (
                                  select column_name
                                  from   public_column_level_access) then lang_comm_int_in_blog_development_description
                  else null
         end as lang_comm_int_in_blog_development_description ,
         case
                  when 'lang_comm_int_in_blog_development_source' in
                           (
                                  select column_name
                                  from   column_level_access) then lang_comm_int_in_blog_development_source
                  when
                           (
                                  select exists
                                         (
                                                ...
                                                where  person =
                                                       (
                                                              ...
                                                              from   admin.tokens
                                                              where  token = :token)
                                                and    role = 1)) then lang_comm_int_in_blog_development_source
                  when owning_person =
                           (
                                  select person
                                  from   admin.tokens
                                  where  token = :token) then lang_comm_int_in_blog_development_source
                  when 'lang_comm_int_in_blog_development_source' in
                           (
                                  select column_name
                                  from   public_column_level_access) then lang_comm_int_in_blog_development_source
                  else null
         end as lang_comm_int_in_blog_development_source ,
         case
...
order by id;""".trimIndent()
        ...
        try {
            ...
            while (jdbcResult.next()) {
                ...
                var is_sign_blog: Boolean? = jdbcResult.getBoolean("is_sign_blog")
                if (jdbcResult.wasNull()) is_sign_blog = null
                ...
                var sign_blog_code: String? = jdbcResult.getString("sign_blog_code")
                if (jdbcResult.wasNull()) sign_blog_code = null
                ...
                var first_blog_population: Int? = jdbcResult.getInt("first_blog_population")
                if (jdbcResult.wasNull()) first_blog_population = null
                ...
                var multiple_blogs_leverage_linguistic_level: String? =
                        jdbcResult.getString("multiple_blogs_leverage_linguistic_level")
                if (jdbcResult.wasNull()) multiple_blogs_leverage_linguistic_level = null

                var multiple_blogs_leverage_linguistic_value: Double? =
                        jdbcResult.getDouble("multiple_blogs_leverage_linguistic_value")
                if (jdbcResult.wasNull()) multiple_blogs_leverage_linguistic_value = null

                var multiple_blogs_leverage_linguistic_description: String? =
                        jdbcResult.getString("multiple_blogs_leverage_linguistic_description")
                if (jdbcResult.wasNull()) multiple_blogs_leverage_linguistic_description = null

                var multiple_blogs_leverage_linguistic_source: String? =
                        jdbcResult.getString("multiple_blogs_leverage_linguistic_source")
                if (jdbcResult.wasNull()) multiple_blogs_leverage_linguistic_source = null

                var multiple_blogs_leverage_joint_training_level: String? =
                        jdbcResult.getString("multiple_blogs_leverage_joint_training_level")
                if (jdbcResult.wasNull()) multiple_blogs_leverage_joint_training_level = null

                var multiple_blogs_leverage_joint_training_value: Double? =
                        jdbcResult.getDouble("multiple_blogs_leverage_joint_training_value")
                if (jdbcResult.wasNull()) multiple_blogs_leverage_joint_training_value = null

                var multiple_blogs_leverage_joint_training_description: String? =
                        jdbcResult.getString("multiple_blogs_leverage_joint_training_description")
                if (jdbcResult.wasNull()) multiple_blogs_leverage_joint_training_description = null

                var multiple_blogs_leverage_joint_training_source: String? =
                        jdbcResult.getString("multiple_blogs_leverage_joint_training_source")
                if (jdbcResult.wasNull()) multiple_blogs_leverage_joint_training_source = null

                var lang_comm_int_in_blog_development_level: String? =
                        jdbcResult.getString("lang_comm_int_in_blog_development_level")
                if (jdbcResult.wasNull()) lang_comm_int_in_blog_development_level = null

                var lang_comm_int_in_blog_development_value: Double? =
                        jdbcResult.getDouble("lang_comm_int_in_blog_development_value")
                if (jdbcResult.wasNull()) lang_comm_int_in_blog_development_value = null

                var lang_comm_int_in_blog_development_description: String? =
                        jdbcResult.getString("lang_comm_int_in_blog_development_description")
                if (jdbcResult.wasNull()) lang_comm_int_in_blog_development_description = null

                var lang_comm_int_in_blog_development_source: String? =
                        jdbcResult.getString("lang_comm_int_in_blog_development_source")
                if (jdbcResult.wasNull()) lang_comm_int_in_blog_development_source = null
                ...
                data.add(
                        blog(
                                ...
                                island = island,
                                province = province,
                                first_blog_population = first_blog_population,
                                ...
                                multiple_blogs_leverage_linguistic_level = if (multiple_blogs_leverage_linguistic_level == null) null else MultipleblogsLeverageLinguisticScale.valueOf(
                                        multiple_blogs_leverage_linguistic_level
                                ),
                                multiple_blogs_leverage_linguistic_value = multiple_blogs_leverage_linguistic_value,
                                multiple_blogs_leverage_linguistic_description = multiple_blogs_leverage_linguistic_description,
                                multiple_blogs_leverage_linguistic_source = multiple_blogs_leverage_linguistic_source,
                                multiple_blogs_leverage_joint_training_level = if (multiple_blogs_leverage_joint_training_level == null) null else MultipleblogsLeverageJointTrainingScale.valueOf(
                                        multiple_blogs_leverage_joint_training_level
                                ),
                                multiple_blogs_leverage_joint_training_value = multiple_blogs_leverage_joint_training_value,
                                multiple_blogs_leverage_joint_training_description = multiple_blogs_leverage_joint_training_description,
                                multiple_blogs_leverage_joint_training_source = multiple_blogs_leverage_joint_training_source,
                                lang_comm_int_in_blog_development_level = if (lang_comm_int_in_blog_development_level == null) null else LangCommIntInblogDevelopmentScale.valueOf(
                                        lang_comm_int_in_blog_development_level
                                ),
                                lang_comm_int_in_blog_development_value = lang_comm_int_in_blog_development_value,
                                lang_comm_int_in_blog_development_description = lang_comm_int_in_blog_development_description,
                                lang_comm_int_in_blog_development_source = lang_comm_int_in_blog_development_source,
