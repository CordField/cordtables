...
data class CommonSiteTextLanguageListResponse (
        val error: ErrorType,
        val data: MutableList<SiteTextLanguage>?
)


@Controller("CommonSiteTextLanguageList")
class List(
        @Autowired
        val siteTextService: SiteTextService,
) {

    @PostMapping("common/site-texts/languages/list")
    @ResponseBody
    fun listHandler(): CommonSiteTextLanguageListResponse {
        var serviceResponse =  siteTextService.getSiteTextLanguages()

        return CommonSiteTextLanguageListResponse(serviceResponse.error, serviceResponse.data)
    }
}