...
data class ScPostsListRequest(
    val token: String?
)

data class ScPostsListResponse(
    val error: ErrorType,
    val posts: MutableList<post>?
)

@CrossOrigin(origins = ["http://localhost:3333", "https://dev.cordtables.com", "https://cordtables.com", "*"])
@Controller("ScPostsList")
class List(
    @Autowired
    val util: Utility,
    ...
    @PostMapping("sc-posts/list")
    @ResponseBody
    fun listHandler(@RequestBody req:ScPostsListRequest): ScPostsListResponse {
        var data: MutableList<post> = mutableListOf()
        if (req.token == null) return ScPostsListResponse(ErrorType.TokenNotFound, mutableListOf())
        ...
        try {
            ...
            return ScPostsListResponse(ErrorType.SQLReadError, mutableListOf())
        }

        return ScPostsListResponse(ErrorType.NoError, data)
    }
}
