    ...
    fun listHandler(@RequestBody req: GroupsListRequest): GroupsListReturn {
        ...
        val listSQL = """      
            ...
            case 
                ...
                then id 
                when (select exists( select id from public.global_role_memberships where person = (select person from public.tokens where token = :token) and global_role = 1)) 
                then id 
                else null 
            end as id,
            case 
            ...
            where id in (select row from row_level_access) or 
                (select exists( select id from public.global_role_memberships where person = (select person from public.tokens where token = :token) and global_role = 1));
        """.trimIndent()
