...
class List(
    ...
    fun listHandler(@RequestBody req: GroupsListRequest): GroupsListReturn {
        ...
        val jdbcResult = jdbcTemplate.queryForRowSet(
            """
                ...
                (
                    ...
                    where a.table_name = 'public.groups'
                    and c.token = :token
                ), 
                ...
                case 
                    ...
                    then id 
                    when (select exists( select id from public.global_role_memberships where person = (select person from public.tokens where token = :token) and id = 1)) 
                    then id 
                    else null 
                end as id,
                case 
                ...
                where id in (select row from row_level_access) or 
                    (select exists( select id from public.global_role_memberships where person = (select person from public.tokens where token = :token) and id = 1));
            """.trimIndent(), paramSource
        )
