...
//            val then = DateTime.now().millis
  ...
  suspend fun createBaseNodeIdempotent(node: BaseNode) {
    when {
      ...
      node.labels.contains("EthnologueLanguage") -> writeEthnologueLanguageBaseNode(
        commonTable = "common.languages",
        sil_table = "sil.language_index",
...
//            println("creating $targetTable node $id")
  ...
  suspend fun writeEthnologueLanguageBaseNode(commonTable: String, sil_table: String, targetTable: String, id: String) {
    val exists = jdbcTemplate.queryForObject(
      "select exists(select id from $targetTable where neo4j_id = ?);",
      Boolean::class.java,
      id
    ...
    if (!exists) {
      ...
      val silId = jdbcTemplate.queryForObject(
        "insert into $sil_table(common_id, created_by, modified_by, owning_person, owning_group) values(?, 1, 1, 1, 1) returning id;",
        Int::class.java,
        commonId,
      )

      jdbcTemplate.update(
        "insert into $targetTable(neo4j_id, language_index, created_by, modified_by, owning_person, owning_group) values(?, ?, 1, 1, 1, 1);",
        id,
        silId
      )
    }
        ...
        .forEach {
          nodePairs.add(
            arrayOf(
              ...
              Relation(
                type = it.get("r").asRelationship().type(),
                active = it.get("r").asRelationship().get("active").toString()
              ),
              createBaseNode(it, "m")
            )
          )
...
//            val then = DateTime.now().millis
  ...
  suspend fun createPropertyNode(node: BaseNode, prop: PropertyNode): MutableMap<String, Exception> {
    ...
    if (prop?.value==null) {
      return err;
    }  // postgres assigns values to null by default, so if they're not in neo4j we stop here.
    if (prop?.type.equals("canDelete")) {
      return err;
    }  // we're not migrating canDelete
    if (prop?.type.equals("roles")) {
      return err;
