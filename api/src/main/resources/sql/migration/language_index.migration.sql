...
BEGIN
  ...
  if vPersonId is not null then
    ...
    values (vPersonId, vPersonId, vPersonId, vGroupId)
    returning id
    into vCommonId;

    insert into sil.language_index(id, lang, country, name_type, name, created_by, modified_by, owning_person, owning_group)
    values (vCommonId, pLang, pCountry, pNameType::sil.language_name_type, pName, vPersonId, vPersonId, vPersonId, vGroupId)
